...
function escolherGrupoPelaHora() {
  const hora = new Date().getHours();
  if (hora >= 5 && hora < 12) {
    return mensagensManha;
  } else if (hora >= 12 && hora < 18) {
    return mensagensTarde;
  } else {
    return mensagensNoite;
  }
}

function iniciarMensagens() {
  if (tocando) return; // Evita clicar duas vezes

  filaMensagens = [...escolherGrupoPelaHora()];
  indiceAtual = 0;
  tocando = true;
...
function tocarProxima() {
  if (indiceAtual >= filaMensagens.length) {
    tocando = false; // Acabou todas
    console.log("Todas as mensagens foram tocadas.");
    return;
  }

  const mensagem = filaMensagens[indiceAtual];
  tocarAudio(mensagem.texto, mensagem.audio);
  indiceAtual++;

  setTimeout(tocarProxima, 7000); // Espera 7 segundos pra próxima
}

// BOTÃO MANUAL
document.getElementById('botaoIniciar').addEventListener('click', iniciarMensagens);
